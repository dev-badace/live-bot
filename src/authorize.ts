 ...
 * }
 */
export async function authorize(options: AuthorizeOptions): Promise<AuthorizeResponse> {
	try {
...
function buildLiveblocksAuthorizeEndpoint(options: AllAuthorizeOptions, roomId: string): string {
	...
	return `https://api.liveblocks.io/v2/rooms/${encodeURIComponent(roomId)}/authorize`;
}
